...
var AUTHORIZATION_KEYS = ['AUTHORIZATION', 'HTTP_AUTHORIZATION', 'X-HTTP_AUTHORIZATION', 'X_HTTP_AUTHORIZATION'];

var Request = exports.Request = function(env) {
    this.env = env;

    if (!this.authorizationKey()) return;
    var parts = HashP.get(env.headers, this._authorizationKey).match(/(\w+) (.*)/);
...
Request.prototype = {
    ...
    authorizationKey: function() {
        ...
        for (var i=0; i < AUTHORIZATION_KEYS.length; i++) {
            ...
            if (HashP.includes(this.env.headers, key)) {
                return this._authorizationKey = key;
            }
        }
    },
